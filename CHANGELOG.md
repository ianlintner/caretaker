...
- reconcile CHANGELOG — 2026-W18 (#612)
- pluggable coding-agent registry + opencode hand-off (#613)
- finalize readiness check when PR is escalated (#614)
- streaming workflow → backend with GitHub OIDC + SSE (#615)
- add v0.23.0 to releases.json (#616)
- harvest hand-off agent reviews into the Reviews tab (#617)
- add v0.24.0 to releases.json (#618)
- fall back to default OIDC audience when env var is empty string (#619)
- two harvest-consumer bugs surfaced by v0.24.0 live QA (#620)
- route fleet workflow to backend with durable eventing + auto-bootstrap (#621)
- qa findings 2026-04-27 (event-bus migration, scenarios 15+16 pass) (#622)
- consolidate dogfood config to .github/maintainer/config.yml (F-5/F-2) (#625)
- add v0.25.0 to releases.json (#626)
- allow auto-approve for maintainer-bot PRs (releases.json) [F-7] (#627)
- upgrade-PR dedup picks highest target version (F-6) + qa-findings 2026-04-27 update (#628)
- server-side dispatch heartbeat — keeps /api/admin/fleet live post-v0.25.0 (#629)
- raise Redis memory limit and add maxmemory eviction (#630)
- end-to-end OpenTelemetry tracing (#631)
- register HTTP metrics middleware at app construction (#632)
- add v0.26.0 to releases.json (#633)
- pre-dispatch comment gate for self-echo + @caretaker triggers (#634)
- use replace --force for the agent-worker Job (#635)
- drop pip cache from thin maintainer workflow (#636)
- add v0.26.1 to releases.json (#637)
- use correct PyPI dist name caretaker-github (#638)
- Update README.md to remove unnecessary image (#639)
- wire InsightStore and auto-discover fleet repos (#643)
- finalize readiness check for untracked closed PRs (#644)
- add v0.26.2 to releases.json (#645)
- retire consumer-side maintainer.yml (#646)
- human PR merge opt-in via label and @caretaker merge comment (#647)
- two-phase triage gate for issue dispatch (prevent dupe/spam queue) (#648)
- pluggable third-party backends + pr-agent integration (#650)
- claude_code_local backend — claude CLI in caretaker pod (#651)
- [WIP] Update documentation to match current state (#653)
- reduce dead code, clean up stubs, rename dedup modules (#654)
- Audit PR flows + 3 follow-ups: rollback wire-up, retry-window symmetry, shadow drift metric (#655)
- LLM consensus engine design (#656)
- orchestrator self-chaining for internal-only transitions (#657)
- LLM consensus engine implementation plan (#658)
- self-heal stale GitHub cooldown when bucket is healthy (#659)
- LLM consensus engine for tiered/multi-model decisions (#660)
- comprehensive tech debt audit (2026-04-30) (#661)
- post-incident hardening (probes, self-heal, admin reset, post-mortem) (#662)
- persist ConsensusTrace onto ShadowDecisionRecord end-to-end (#663)
- make rate-limit cooldown gauge live at scrape time (#664)
- OpenRouter integration with per-feature routing + :online web grounding (#665)
- add v0.27.0 to releases.json (#666)
- switch dogfood LLM routing to provider=openrouter (#667)
- enforce ownership requirement for review fix requests in PRAgent (#668)
