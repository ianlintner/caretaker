...
class PRAgent:
    ...
    async def _post_merge_summary_comment(self, pr: PullRequest) -> None:
        """Post a close-out summary comment on the PR just before merging.

        When the LLM is available the comment body is generated by
        :meth:`~caretaker.llm.claude.ClaudeClient.generate_merge_summary` and
        includes a summary of the changes, interesting notes, and follow-up
        items. Falls back to a plain template when the LLM is unavailable or
        the call fails. The comment is best-effort — a failure here never
        blocks the merge.
        """
        summary: str | None = None
        if self._llm and self._llm.available:
            try:
                summary = await self._llm.claude.generate_merge_summary(
                    pr.number,
                    pr.title,
                    pr.body or "",
                    pr.user.login,
                ...
                logger.warning(
                    "PR #%d: merge summary generation failed, falling back to template",
                    pr.number,
                    exc_info=True,
                )

        if not summary:
            summary = f"Merging **{pr.title}** (#{pr.number})."

        body = f"<!-- caretaker:merge-summary -->\n{summary}"
        try:
            await self._github.add_issue_comment(self._owner, self._repo, pr.number, body)
        except Exception:
            logger.warning(
                "PR #%d: failed to post merge summary comment",
                pr.number,
                exc_info=True,
            )
    ...
    async def _handle_merge(
        ...
        await self._post_merge_summary_comment(pr)
