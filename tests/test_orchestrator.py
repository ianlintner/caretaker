...
class TestOrchestratorReportPath:
    ...
    async def test_self_heal_mode_runs_only_self_heal_agent(
        self, tmp_path: pathlib.Path
    ) -> None:
        """When mode='self-heal', only the self-heal agent is invoked."""
        orchestrator = make_orchestrator()
        ...
        with (
            ...
            await orchestrator.run(
                mode="self-heal", event_type="workflow_run", event_payload=payload
            )
