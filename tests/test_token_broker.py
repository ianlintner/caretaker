...
class TestBuildTokenBroker:
    def test_returns_none_when_app_id_not_set(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("CARETAKER_GITHUB_APP_ID", raising=False)
        broker = build_token_broker()
        assert broker is None

    def test_returns_none_when_app_id_not_integer(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CARETAKER_GITHUB_APP_ID", "not-an-int")
        broker = build_token_broker()
        assert broker is None

    def test_returns_none_when_no_private_key(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_minter_when_configured(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_uses_in_process_cache_without_redis(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CARETAKER_GITHUB_APP_ID", "42")
    ...
    def test_uses_redis_cache_when_redis_url_set(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("CARETAKER_GITHUB_APP_ID", "42")
        monkeypatch.setenv("CARETAKER_GITHUB_APP_PRIVATE_KEY", "fake-pem")
